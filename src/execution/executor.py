        ...
        for trade_uuid, order in list(self._open_orders.items()):
            try:
                ...
                try:
                    ...
                    updated    = analysis_engine.analyze_market(market_dict)
                    model_prob = (updated.predicted_prob if updated
                                  else (yes_p if outcome == "YES" else no_p))
                except Exception:
                    model_prob = yes_p if outcome == "YES" else no_p
